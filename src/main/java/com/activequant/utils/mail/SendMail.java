...
public class SendMail {
	...
	public void sendMail(String[] recipients, String subject, String htmlBody, String[] attachmentFileNames)
			throws Exception {
		Properties props = new Properties();
		props.setProperty("mail.transport.protocol", "smtp");
		props.setProperty("mail.host", host);
		...
		for (int i = 0; i < attachmentFileNames.length; i++) {
			...
			DataSource fds = new FileDataSource(attachmentFileNames[i]);
			messageBodyPart.setDataHandler(new DataHandler(fds));
			messageBodyPart.setHeader("Content-ID", "ATTACHMENT" + i);
			multipart.addBodyPart(messageBodyPart);
		}
