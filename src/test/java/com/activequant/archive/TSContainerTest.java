...
public class TSContainerTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		timeSeries = new TSContainer2(TSContainerTest.class.getName(), Arrays.asList(headers), columns);
		timeSeries.setRow(new TimeStamp(new Date(1336305521100L)), new Double[] {2.0, 2.1, 2.2, 2.3});
		timeSeries.setRow(new TimeStamp(new Date(1336305521300L)), new Double[] {4.0, 4.1, 4.2, 4.3});
		timeSeries.setRow(new TimeStamp(new Date(1336305521000L)), new Double[] {1.0, 1.1, 1.2, 1.3});
		timeSeries.setRow(new TimeStamp(new Date(1336305521200L)), new Double[] {3.0, 3.1, 3.2, 3.3});
	}
	...
	@Test
	public void testAdd() {
		timeSeries.setRow(new TimeStamp(new Date(1336305521250L)), new Double[] {3.0, 3.1, 3.2, 3.3});
	}
	...
	public void testDeleteFromTo() {
		timeSeries.delete(new TimeStamp(new Date(1336305521000L)), new TimeStamp(new Date(1336305521200L)));
		
		assertEquals(1, timeSeries.getNumRows());
		assertEquals(1336305521300L, timeSeries.getTime(0).getMilliseconds());
		assertEquals(4.0, timeSeries.getRow(new TimeStamp(new Date(1336305521300L)))[0]);
	}
	
	@Test
	public void testGetIndexBeforeOrEqual() {
		assertEquals(2, timeSeries.getIndexBeforeOrEqual(new TimeStamp(new Date(1336305521200L))));
		assertEquals(2, timeSeries.getIndexBeforeOrEqual(new TimeStamp(new Date(1336305521250L))));
	}
	
	@Test
	public void testGetIndexAfter() {
		assertEquals(3, timeSeries.getIndexAfter(new TimeStamp(new Date(1336305521200L))));
		assertEquals(3, timeSeries.getIndexAfter(new TimeStamp(new Date(1336305521250L))));
	}
	...
	public void testGetMinTimestamp() {
		assertEquals(new TimeStamp(new Date(1336305521000L)), timeSeries.getMinTimestamp());
	}

	@Test
	public void testGetMaxTimestamp() {
		assertEquals(new TimeStamp(new Date(1336305521300L)), timeSeries.getMaxTimestamp());
	}
	
	@Test
	public void testSetWindowSize() throws InterruptedException {
		timeSeries.delete(new TimeStamp(new Date(1336305521000L)), new TimeStamp(new Date(1336305521300L)));
		...
		for(int i=0; i<7; i++) {
			timeSeries.setRow(new TimeStamp(new Date()), 2.0+i, 2.1+i, 2.2+i, 2.3+i);
			Thread.sleep(1);
		}
	...
	public void testDeleteBeforeTimeStamp() throws InterruptedException {
		...
		for(int i=0; i<5; i++) {
			timeSeries.setRow(new TimeStamp(new Date()), 1.0+i, 1.1+i, 1.2+i, 1.3+i);
			Thread.sleep(1);
		}
		...
		for(int i=0; i<4; i++) {
			timeSeries.setRow(new TimeStamp(new Date()), 7.0+i, 7.1+i, 7.2+i, 7.3+i);
			Thread.sleep(1);
		}
	...
	public void testDeleteAfterIndex() throws InterruptedException {
		...
		assertEquals(ts2, timeSeries.getTime(timeSeries.getIndex(timeSeries.getMaxTimestamp())));
		...
		timeSeries.deleteAfter(0);
		assertEquals(0, timeSeries.getNumRows());
		// assertEquals(ts1, timeSeries.getTime(timeSeries.getIndex(timeSeries.getMaxTimestamp())));
	}
}
